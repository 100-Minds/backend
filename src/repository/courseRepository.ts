...
class CourseRepository {
	...
	//lesson
	getAllCoursesWithLessons = async (): Promise<ILesson[] | null> => {
		const courses = await knexDb
			.table('course')
		...
		const chapters = await knexDb('course_chapters')
			.select('id', 'title', 'description', 'chapterNumber', 'chapterResources', 'courseId', 'created_at')
		...
		const chaptersByCourse: { [key: string]: ICourseChapter[] } = {};
		...
		// Fetch all videos for the chapters
		...
		// Construct the final lessons array
		const allLessons: ILesson[] = courses.map((course) => ({
			course: course as ICourse,
			chapters: (chaptersByCourse[course.id] || []).map((chapter) => ({
				id: chapter.id,
				title: chapter.title,
