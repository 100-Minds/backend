...
class LastWatchedRepository {
    ...
    findByVideoAndCourse = async (userId: string, videoId: string, courseId: string): Promise<ILastWatched[]> => {
		return await knexDb.table('last_watched').where({ userId, videoId, courseId }).returning('*');
	}
