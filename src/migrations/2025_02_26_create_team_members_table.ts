...
export async function up(knex: Knex): Promise<void> {
	return knex.schema.createTable('team_members', (table) => {
		...
		table.uuid('teamId').references('id').inTable('teams').onDelete('CASCADE'); // Team reference
		table.uuid('userId').references('id').inTable('users').onDelete('CASCADE'); // User reference
